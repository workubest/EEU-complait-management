 ...
 * 
 * Deployment URL: https://script.google.com/macros/s/AKfycbwRtSTJjIA9_Hx-SpX95dJ2hRg1SSkEGLlyqjWElWJoiGQWtLzt7pwYeyeycah7KpI/exec
 */

// --- CONFIGURATION ---
const CONFIG = {
  SHEET_ID: '1vi7SguM67N8BP5_5dNSPh4GO0WDe-Y6_LwfY-GliW0o', // Update with your Google Sheets ID
  USER_SHEET: 'Users',
    ...
    if (action) {
      switch (action) {
        ...
        case 'createCustomer':
          ...
          break;
        case 'searchCustomer':
          result = searchCustomerInternal(params);
          break;
...
function getUsersInternal(params) {
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      delete user.Password; // Don't return passwords
      users.push(user);
    }
...
function searchCustomerInternal(params) {
  try {
    const searchType = params.type; // 'contract' or 'business'
    const searchValue = params.value;
    
    if (!searchType || !searchValue) {
      return { error: 'Missing search parameters', success: false };
    }
    ...
    const data = sheet.getDataRange().getValues();
    if (!data || data.length === 0) {
      return { error: 'No customers found', success: false };
    }
    
    const headers = data[0];
    const searchColumn = searchType === 'contract' ? 'Account Number' : 'Name';
    const searchIdx = headers.indexOf(searchColumn);
    
    if (searchIdx === -1) {
      return { error: `${searchColumn} column not found`, success: false };
    }
    
    for (let i = 1; i < data.length; i++) {
      const row = data[i];
      if (row[searchIdx] && row[searchIdx].toString().toLowerCase().includes(searchValue.toLowerCase())) {
        let customer = {};
        for (let j = 0; j < headers.length; j++) {
          customer[headers[j]] = row[j] || '';
        }
    ...
    return { error: 'Failed to search customer: ' + error.message, success: false };
  }
}
